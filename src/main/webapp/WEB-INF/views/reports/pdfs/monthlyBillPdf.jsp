...
<title>Month Wise Bill Report</title>
<style type="text/css">
@media print {
	.no-break {
...
<body>
					<div class="no-break">
						 ...
						 <div
							...
							<h4
								style="color: #000; font-size: 16px; text-align: center; margin: 0px;">Month Wise Bill Report</h4>
							
								<p
									style="color: #000; font-size: 15px; text-align: center; font-weight: bold;">
									Months : ${fromMonth}&nbsp;&nbsp; to &nbsp;&nbsp;${toMonth}</p>
				...
				style="border-top: 1px solid #313131;" class="table datatable-header-basic">
					<tr>				
						...
						<th width="10">Year</th>
						<th width="10%">No. Of Bills</th>
						<th width="20%">Total Amt</th>		
						<th width="10">COD</th>	
						<th width="10">Card</th>
						<th width="10">E-Pay</th>
					</tr>
					...
					<c:forEach items="${monthBillRep}" var="order" varStatus="count">
							<tr>
								<td width="5%">${srno}</td>
								<td width="10%" style="text-align: center;">${order.monthName}</td>
								<td width="10%" style="text-align: center;">${order.orderYear}</td>
								<td width="10%" style="text-align: right;">${order.totalBills}</td>
								<td width="15%" style="text-align: right;">${order.totalAmt}</td>								
								<td width="15%" style="text-align: right;">${order.cod}</td>								
								<td width="15%" style="text-align: right;">${order.card}</td>	
								<td width="15%" style="text-align: right;">${order.epay}</td>										
							</tr>

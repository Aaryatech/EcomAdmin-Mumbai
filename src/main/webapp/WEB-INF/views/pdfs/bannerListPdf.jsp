...
<body>
					<div class="no-break">
						...
						<div
							...
							<h4
								style="color: #000; font-size: 16px; text-align: center; margin: 0px;">Banner List Pdf</h4>							
								<%-- <p
		...
		<table align="center" border="1" cellpadding="0" cellspacing="0"
			...
			<tr>
				<c:forEach items="${bannerIds}" var="bannerIds" varStatus="count">

					<c:if test="${bannerIds==1}">
						<th width="10%">Sr. No.</th>
					</c:if>

					<c:if test="${bannerIds==2}">
						<th>Event Name</th>
					</c:if>

					<c:if test="${bannerIds==3}">
						<th>Sort No.</th>
					</c:if>

					<c:if test="${bannerIds==4}">
						<th>Caption on Product Page</th>
					</c:if>

					<c:if test="${bannerIds==5}">
						<th>Status</th>
					</c:if>

					<c:if test="${bannerIds==6}">
						<th>Franchise</th>
					</c:if>
					
					<c:if test="${bannerIds==7}">
						<th>Tags</th>
					</c:if>
				</c:forEach>
			</tr>
			...
			<c:forEach items="${bannerPrintList}" var="bannerPrintList" varStatus="count">
				<tr>
					<c:forEach items="${bannerIds}" var="bannerIds">
					...
					<c:if test="${bannerIds==2}">						
						<td>${bannerPrintList.bannerEventName}</td>
					</c:if>
					
					<c:if test="${bannerIds==3}">						
					<td>${bannerPrintList.sortNo}</td>
					</c:if>
					
					<c:if test="${bannerIds==4}">						
						<td>${bannerPrintList.captionOnproductPage}</td>
					</c:if>
					
					<c:if test="${bannerIds==5}">						
						<td>${bannerPrintList.isActive==1 ? 'Active' : 'In-Active'}</td>
					</c:if>
					
					<c:if test="${bannerIds==6}">						
						<td>${bannerPrintList.exVar2}</td>
					</c:if>
					
					<c:if test="${bannerIds==7}">						
						<td>${bannerPrintList.exVar3}</td>
					</c:if>			

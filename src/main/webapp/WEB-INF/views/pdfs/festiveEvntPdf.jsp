...
<title>Configure Products And Festive Event</title>
<style type="text/css">
@media print {
	.no-break {
...
<body>
					<div class="no-break">
						...
						<div
							...
							<h4
								style="color: #000; font-size: 16px; text-align: center; margin: 0px;">Configure Products And Festive Event </h4>							
								<%-- <p
		...
		<table align="center" border="1" cellpadding="0" cellspacing="0"
			...
			<tr>				
				<c:forEach items="${configEvntIds}" var="configEvntIds">
					<c:if test="${configEvntIds==1}">
						<th>Sr No.</th>
					</c:if>
					
					<c:if test="${configEvntIds==2}">
						<th>Event Name</th>
					</c:if>
					
					<c:if test="${configEvntIds==3}">
						<th>Period</th>
					</c:if>
					
					<c:if test="${configEvntIds==4}">
						<th>Time</th>
					</c:if>
					
					<c:if test="${configEvntIds==5}">
						<th>Status</th>
					</c:if>
				</c:forEach>
			</tr>
			...
			<c:forEach items="${festiveEventList}" var="event" varStatus="count">
				<tr>
					<c:forEach items="${configEvntIds}" var="configEvntIds">

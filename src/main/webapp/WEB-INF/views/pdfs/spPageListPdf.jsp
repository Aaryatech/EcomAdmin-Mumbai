...
<title>SP Day Home Page List List</title>
<style type="text/css">
@media print {
	.no-break {
...
<body>
					<div class="no-break">
						...
						<div
							...
							<h4
								style="color: #000; font-size: 16px; text-align: center; margin: 0px;">SP Day Home Page List Pdf</h4>							
								<%-- <p
		...
		<table align="center" border="1" cellpadding="0" cellspacing="0"
			...
			<tr>
				<c:forEach items="${spIds}" var="spIds" varStatus="count">
			...
			<c:forEach items="${spPageListPrint}" var="spPageListPrint" varStatus="count">
				<tr>
					<c:forEach items="${spIds}" var="spIds">
					...
					<c:if test="${spIds==2}">						
						<td>${spPageListPrint.spdayName}</td>
					</c:if>
					
					<c:if test="${spIds==3}">						
					<td>${spPageListPrint.spdayCaptionHomePage}</td>
					</c:if>
					
					<c:if test="${spIds==4}">						
						<td>${spPageListPrint.franchises}</td>
					</c:if>
					
					<c:if test="${spIds==5}">						
						<td>${spPageListPrint.filterName}</td>
					</c:if>
					
					<c:if test="${spIds==6}">						
						<td>${spPageListPrint.fromDate} to ${spPageListPrint.toDate}</td>
					</c:if>
					
					<c:if test="${spIds==7}">						
						<td>${spPageListPrint.fromTime} to ${spPageListPrint.toTime}</td>
					</c:if>	
					
					<c:if test="${spIds==8}">						
						<td>${spPageListPrint.sortNo}</td>
					</c:if>	
					
					<c:if test="${spIds==9}">						
						<td>${spPageListPrint.isActive==1 ? 'Active' : 'In-Active'}</td>
					</c:if>			

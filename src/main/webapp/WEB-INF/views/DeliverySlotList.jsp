...
<c:url  value="/deleteSelMultiSlots" var="deleteSelMultiSlots"/> 
	<!-- Main navbar -->
				...
				<div class="card-body">
					...
					<div
						...
						<span
							...
							Delivery Slot List</span>
						<!--  -->
						<c:if test="${addAccess == 1}">
							<span class="font-size-sm text-uppercase font-weight-semibold"><a
								class="card-title"
								href="${pageContext.request.contextPath}/AdddeliverySlot"
					...
					<table class="table datatable-header-basic" id="printtable">
						...
						<tbody>
							<c:forEach items="${delSlotList}" var="delSlotList" varStatus="count">
								<tr>
									<td>${count.index+1}   &nbsp;
									<input type="checkbox" id="city${delSlotList.deliSlotId}" value="${delSlotList.deliSlotId}" name="cityId" class="chkcls"></td>
									<td>${delSlotList.deliverySlotName}</td>
									<td>${delSlotList.fromTime}</td>
									<td>${delSlotList.toTime}</td>
									<c:choose>
										<c:when test="${delSlotList.isActive==1}">
								...
								<%-- 	<td>${custList.cityName}</td>
									...
									<td class="text-center"><c:if test="${editAccess == 1 }">
											<div class="list-icons">
												<a
													href="${pageContext.request.contextPath}/editdeliverySlot?delSlotId=${delSlotList.deliSlotId}"
											...
											<div class="list-icons">
												<a href="javascript:void(0)"
													...
													data-uuid="${delSlotList.deliSlotId}" data-popup="tooltip"
							...
							bootbox
									.confirm({
										...
										callback : function(result) {
											if (result) {
												location.href = "${pageContext.request.contextPath}/deletedeliverySlot?delSlotId/*  */="
														+ uuid;

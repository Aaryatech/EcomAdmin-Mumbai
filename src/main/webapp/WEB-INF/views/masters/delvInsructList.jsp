...
<c:url value="deleteSelMultiDelIns" var="deleteSelMultiDelIns"></c:url>
	<!-- Main navbar -->
					...
					<table class="table datatable-header-basic" id="printtable">
						...
						<tbody>
							<c:forEach items="${delvList}" var="delvList" varStatus="count">
								<tr>
									<td>${count.index+1}   &nbsp;
									<input type="checkbox" id="city${delvList.instruId}" value="${delvList.instruId}" name="cityId" class="chkcls"></td>
					...
					.confirm({
						...
						callback : function(
							...
							if (result) {
								...
								$
								.getJSON(
										'${deleteSelMultiDelIns}',
										{
											insIds : JSON.stringify(insIds),
											ajax : 'true'
										},
										function(data) {
											if(!data.error){
	...
	function genPdf() {
		...
		window
				.open("${pageContext.request.contextPath}/pdfReport?url=pdf/getDlvrInstListPdf/"+compId+"/"+showHead);
	}
	</script>
</body>
</html>
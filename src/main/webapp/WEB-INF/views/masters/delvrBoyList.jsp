...
<c:url value="deleteSelMultiDeliveryBoy" var="deleteSelMultiDeliveryBoy"></c:url>
	<!-- Main navbar -->
			...
			<div class="content">
				...
				<div class="card">
					...
					<div
						...
						<c:if test="${addAccess==0}">
							<span class="font-size-sm text-uppercase font-weight-semibold"><a
								class="card-title"
								href="${pageContext.request.contextPath}/newDeliveryBoy"
								style="color: white;"><i class="icon-add-to-list ml-2"
									style="font-size: 23px;"></i>&nbsp;&nbsp;&nbsp;&nbsp;Add Delivery Boy</a></span>
						</c:if>
					</div>
					...
					<table class="table datatable-header-basic" id="printtable">
						...
						<tbody>
							<c:forEach items="${delBoyList}" var="list" varStatus="count">
								<tr>
									<td>${count.index+1}   &nbsp;
									<input type="checkbox" id="${list.delBoyId}" value="${list.delBoyId}" name="cityId" class="chkcls"></td>
									...
									<td class="text-center"><c:if test="${editAccess==0}">
											<div class="list-icons">
												<a
													href="${pageContext.request.contextPath}/editDeliveryBoy?delBoyId=${list.exVar1}"
							...
							bootbox
									.confirm({
										...
										callback : function(result) {
											if (result) {
												location.href = "${pageContext.request.contextPath}/deleteDeliveryBoy?delBoyId="
														+ uuid;
					...
					.confirm({
						...
						callback : function(
							...
							if (result) {
								...
								$
								.getJSON(
										'${deleteSelMultiDeliveryBoy}',
										{
											dBoyIds : JSON.stringify(dBoyIds),
											ajax : 'true'
										},
										function(data) {
											//alert(data.error);
	...
	function genPdf() {
		...
		window
				.open("${pageContext.request.contextPath}/pdfReport?url=pdf/getDeliveBoyPdf/"+compId+"/"+showHead);
	}
	</script>
</body>
</html>
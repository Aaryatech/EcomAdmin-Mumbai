...
<c:url value="deleteSelMultiFiltertype" var="deleteSelMultiFiltertype"></c:url>
	<!-- Main navbar -->
					...
					<div
						...
						<c:if test="${addAccess==0}">
							<span class="font-size-sm text-uppercase font-weight-semibold"><a
								class="card-title"
								href="${pageContext.request.contextPath}/newFilterType"
					...
					<table class="table datatable-header-basic" id="printtable">
						...
						<tbody>
							<c:forEach items="${filterList}" var="filterList" varStatus="count">
								<tr>
									<td>${count.index+1}   &nbsp;
									<input type="checkbox" id="${filterList.filterTypeId}" value="${filterList.filterTypeId}" name="cityId" class="chkcls"></td>
									<td>${filterList.filterTypeName}</td>
									<td>${filterList.isActive==1 ? 'Active' : 'In-Active'}</td>
									<td>${filterList.isCostAffect==1 ? 'Yes' : 'No'}</td>
									<td>${filterList.isUsedFilter==1 ? 'Yes' : 'No'}</td>
									<td class="text-center"><c:if test="${editAccess==0}">
											<div class="list-icons">
												<a
													href="${pageContext.request.contextPath}/editFilterType?filterTypeId=${filterList.exVar1}"
							...
							bootbox
									.confirm({
										...
										callback : function(result) {
											if (result) {
												location.href = "${pageContext.request.contextPath}/deleteFilterType?filterTypeId="
														+ uuid;
					...
					.confirm({
						...
						callback : function(
							...
							if (result) {
								...
								$
								.getJSON(
										'${deleteSelMultiFiltertype}',
										{
											fTypeIds : JSON.stringify(fTypeIds),
											ajax : 'true'
										},
										function(data) {
											//alert(data.error);

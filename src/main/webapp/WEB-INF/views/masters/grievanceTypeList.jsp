...
<c:url value="deleteSelMultiGrievances" var="deleteSelMultiGrievances"></c:url>
	<!-- Main navbar -->
					...
					<div
						...
						<c:if test="${addAccess==0}">
						 <span
							...
							href="${pageContext.request.contextPath}/addGrievanceTypInstruct"
							style="color: white;"><i
								class="icon-add-to-list ml-2" style="font-size: 23px;"></i>&nbsp;&nbsp;&nbsp;&nbsp;Add Grievances Type Instruction</a></span>
						</c:if>
					</div>
					...
					<table class="table datatable-header-basic" id="printtable">
						...
						<tbody>
							<c:forEach items="${grievList}" var="grievList" varStatus="count">
								 <tr>
									<td>${count.index+1}   &nbsp;
									<input type="checkbox" id="city${grievList.grevTypeId}" value="${grievList.grevTypeId}" name="cityId" class="chkcls"></td>
									...
									<c:if test="${editAccess==0}">
										<div class="list-icons">
											<a href="${pageContext.request.contextPath}/editGrievanceTypeInsrtuctn?grievId=${grievList.exVar1}"
...
$('.bootbox_custom')
				...
				function() {
					var uuid = $(this).data("uuid") // will return the number 123
								...
								callback : function(result) {
									if (result) {
										location.href = "${pageContext.request.contextPath}/deleteGrievanceType?grievId="
												+ uuid;
	...
	function genPdf() {
		...
		window
				.open("${pageContext.request.contextPath}/pdfReport?url=pdf/getGrievInstListPdf/"+compId+"/"+showHead);
	}
	</script>
</body>
</html>
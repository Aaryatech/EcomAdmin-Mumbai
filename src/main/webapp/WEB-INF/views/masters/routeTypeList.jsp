...
<body class="sidebar-xs">
	...
	<div class="page-content">
		...
		<!-- /main sidebar -->
	<c:url var="deleteSelRoutesType" value="deleteSelRoutesType"></c:url>
			...
			<div class="content">
				...
				<div class="card">
					...
					<div
						...
						<c:if test="${addAccess==0}">
							<span class="font-size-sm text-uppercase font-weight-semibold"><a
								class="card-title"
								href="${pageContext.request.contextPath}/showAddRouteType"
								style="color: white;"><i class="icon-add-to-list ml-2"
									style="font-size: 23px;"></i>&nbsp;&nbsp;&nbsp;&nbsp;Add Route Type
							</a></span>
						</c:if>
					</div>
					...
					<table class="table datatable-header-basic" id="printtable">
						...
						<tbody>
							<c:forEach items="${routeTypeList}" var="routeList" varStatus="count">
								<tr>
									<td>${count.index+1}   &nbsp;
									<input type="checkbox" id="${routeList.routeTypeId}" value="${routeList.routeTypeId}" name="routeId" class="chkcls"></td>
									<td>${routeList.routeTypeName}</td>
									<td>${routeList.routeRangeInKm}</td>
									<td>${routeList.sortNo}</td>
									 
									<td class="text-center"><c:if test="${editAccess==0}">
											<div class="list-icons">
												<a
													href="${pageContext.request.contextPath}/showEditRouteType?routeTypeId=${routeList.exVar1}"
					...
					<span class="validation-invalid-label" id="error_chks"
						...
						<c:choose>
					<c:when test="${routeTypeListSize<=0}">
							...
							bootbox
									.confirm({
										...
										callback : function(result) {
											if (result) {
												location.href = "${pageContext.request.contextPath}/deleteRouteType?routeTypeId="
														+ uuid;
	...
	function genPdf() {
		...
		window
				.open("${pageContext.request.contextPath}/pdfReport?url=pdf/getRouteTpyePdf/"+compId+"/"+showHead);
	}
	</script>
</body>
</html>
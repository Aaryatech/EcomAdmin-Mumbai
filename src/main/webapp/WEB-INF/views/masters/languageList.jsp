...
<c:url value="deleteSelMultiLanguages" var="deleteSelMultiLanguages"></c:url>
	<!-- Main navbar -->
			...
			<div class="content">
				...
				<div class="card">
					...
					<table class="table datatable-header-basic" id="printtable">
						...
						<tbody>
							<c:forEach items="${langList}" var="langList" varStatus="count">
								<tr>
									<td>${count.index+1}   &nbsp;
									<input type="checkbox" id="city${langList.langId}" value="${langList.langId}" name="cityId" class="chkcls"></td>
									<td>${langList.langCode}</td>
									<td>${langList.langName}</td>
									<c:set value="" var="status" />
									<c:if test="${langList.isActive==1}">
										<c:set value="Active" var="status" />
									</c:if>
									<c:if test="${langList.isActive==0}">
									...
									<c:if test="${editAccess==0}">
										<div class="list-icons">
											<a
												href="${pageContext.request.contextPath}/editLang?langId=${langList.exVar1}"
										...
										<div class="list-icons">
											<a href="javascript:void(0)"
												...
												data-uuid="${langList.exVar1}" data-popup="tooltip" title=""
							...
							bootbox
									.confirm({
										...
										callback : function(result) {
											if (result) {
												location.href = "${pageContext.request.contextPath}/deleteLang?langId="
														+ uuid;
					...
					.confirm({
						...
						callback : function(
							...
							if (result) {
								...
								$
								.getJSON(
										'${deleteSelMultiLanguages}',
										{
											langIds : JSON.stringify(langIds),
											ajax : 'true'
										},
										function(data) {
											if(!data.error){
	...
	function genPdf() {
		...
		window
				.open("${pageContext.request.contextPath}/pdfReport?url=pdf/getLanguageListPdf/"+compId+"/"+showHead);
	}
	</script>
</body>
</html>